...
export default Ember.Mixin.create({
    ...
    actions:{
        ...
        unloadArchivedPieces(){
            let store = this.store;
            this.get("model").filterBy("is_archived",true).forEach((piece)=>{
                store.unloadRecord(piece);
            });
        },
        fetchArchivedPieces(searchKey){
            if(!searchKey){
                searchKey = "";
            }
            ...
            this.store.query('piece',{
                key:searchKey,
                count:pageCount,
                lastId:lastId
            }).then((answer)=>{
                let pieces = answer.toArray();
                if(pieces.length > 0){
                    let minId = pieces.mapBy("id").sort(function(a,b){
                        return parseInt(a) - parseInt(b);
                    }).get("firstObject");
                    this.set("lastId",minId);
                }
                if(pieces.length < pageCount){
                    this.set("isMoreButtonNeeded",false);
                }
                else{
        ...
        fetchNext(){
            ...
            if(this.get("isSearchkeyEntered")){
                this.send("fetchArchivedPieces",this.get("searchKey"));
            }
            else{
                this.send("fetchArchivedPieces");
            }
        },
        enterSearch(searchKey){
            ...
            if(this.get("isEmptyKeyFetchable")){
	            this.send("fetchArchivedPieces",searchKey);
            }
